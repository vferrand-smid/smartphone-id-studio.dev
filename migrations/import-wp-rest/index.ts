...
if (!SANITY_LOCALE)
  throw new Error(`❌ Locale WP "${LANGUAGE_CODE}" inconnue dans le mapping.`);
...
export default defineMigration({
  ...
  async *migrate() {
    ...
    console.log(
      `🚀 Migration REST pour WP=${LANGUAGE_CODE} → Sanity=${SANITY_LOCALE}`
    );
          ...
          limit(async () => {
            try {
              ...
              const slug = decodeURIComponent(
                pageItem.slug || pageItem.id.toString()
              );
